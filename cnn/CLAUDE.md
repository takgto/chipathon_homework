# cnn/ ―― Claude Code への指示

半導体デザインハッカソン（KV260 + YOLOv3）の前に学生が取り組む **CNN 事前学習用問題集**（第1〜5章、問1〜問14）です。
第4章は Google Colab 上で LeNet を学習・推論します。リポジトリ **takgto/chipathon_homework** の1フォルダ（同じリポジトリに `../linux_command/`、`../cpp/`。全体のルールはトップの `CLAUDE.md`）。C++ 並行処理演習は別リポジトリ **takgto/cpp-lab**。
...
```bash
python3 gen/build_cnn.py     # どこからでも可。出力先はこのフォルダ直下。追加ライブラリ不要
```
